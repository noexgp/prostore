...
export async function createOrder({ cashTendered }: { cashTendered: number }) {
  try {
    ...
    if (user.paymentMethod === 'Cash') {
      xchangedue = cashTendered - Number(cart.totalPrice)
    } else {
      xchangedue = 0.0
      cashTendered = 0.0
    }
    ...
    const order = insertOrderSchema.parse({
      ...
      changeDue: xchangedue.toFixed(2),
    })
    ...
    if (user.paymentMethod === 'Cash' || user.paymentMethod === 'GCASH') {
      await updateOrderToPaidCOD(insertedOrderId)
    }
...
async function updateOrderToPaid({
  ...
  await prisma.$transaction(async (tx) => {
    //Iterate over products and upate stock
...
export async function getOrderSummary() {
  ...
  >`SELECT to_char("createdAt",'MM/YY') as "month", sum("totalPrice") as "totalSales" FROM "Order" GROUP BY to_char("createdAt",'MM/YY') ORDER BY  1`
